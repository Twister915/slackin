...
export default function splash({ name, logo, active, total, channels, iframe, locked }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    dom('form',
      ...
      dom('input.form-item type=email placeholder=you@yourdomain.com '
        + (!iframe ? 'autofocus' : '')),
      locked ? dom('button.locked', 'Currently Locked!') : dom('button.loading', 'Get my Invite')
    ),
    !iframe && dom('footer',
      'powered by ',
